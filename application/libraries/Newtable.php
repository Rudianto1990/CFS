...
class Newtable {
	...
	function tipe_proses2($show)
	{
		$this->tipe_proses2 = $show;
		return;
	}
	...
	function numberformat($row)
	{
		...
		foreach ( $row as $a )
		{
			if( ! in_array($a, $this->numberformat)) $this->numberformat[] = $a;
		}
		return;
	}
	...
	function generate($table_data = NULL){
					...
					if($this->multiple_search){
						foreach($keys as $a => $b){
							...
							if($type=="DATERANGE"){
								...
								if(count($cari)>0){
									if(($cari[0]!="")&&($cari[1]!="")){
										$tercari .= " AND DATE_FORMAT($field,'%Y%m%d') BETWEEN DATE_FORMAT(STR_TO_DATE('$cari[0]','%d-%m-%Y'),'%Y%m%d') AND DATE_FORMAT(STR_TO_DATE('$cari[1]','%d-%m-%Y'),'%Y%m%d')";
									}else if($cari[0]!=""){
							...
							}else if($type=="STATICDATERANGE"){
								if($cari[0]!="" && $cari[1]!=""){
									$awal = explode("-",$cari[0]);
									$startofdate = $awal[0]." ".$cari[1];
									$startdate = date("d-m-Y", strtotime($startofdate));
									if($awal[1]=="end"){
										$enddate = date("t-m-Y", strtotime($startofdate));
									}else{
										$enddate = date("d-m-Y", strtotime($awal[1]." ".$cari[1]));
									}
									$tercari .= " AND DATE_FORMAT($field,'%d-%m-%Y') BETWEEN '$startdate' AND '$enddate'";
								}
			...
			if($this->multiple_search){
				foreach ($this->keycari as $a => $b){
					if($b[2]=="DATERANGE"){
						...
						$out .=	'	</div>';
						$out .=	'</div>';
					}else if($b[2]=="STATICDATERANGE"){
						$out .=	'<div class="form-group">';
						$out .=	'<label class="col-sm-2 control-label-left">'.$b[1].'</label>';
						$out .=	'	<div class="col-sm-5">';
						$out .=	'	<select name="form['.$a.'][]" id="'.rand(pow(10,$a)).'" class="form-control">';
						$rangetgl = array(""=>"","1-7"=>"1 - 7","8-14"=>"8 - 14","15-21"=>"15 - 21","22-end"=>"22 - end");
						foreach($rangetgl as $opt => $valopt){
							$selected_opt = "";
						...
						$out .=	'	</div>';
						$out .=	'	<div class="col-sm-5">';
						$out .=	'		<input type="text" name="form['.$a.'][]" id="'.rand(pow(10,$a)).'" class="mdrp form-control '.$a.'" value="'.$arrvalcari[$a][1].'" placeholder="MONTH-YEAR" tag="'.$b[2].'" readonly>';
						$out .=	'	</div>';
						$out .=	'</div>';
					}else if($b[2]=="TESDATE"){
						$out .=	'<div class="form-group">';
				...
				$out .=	'	</div>';
				$out .=	'</div>';
				$out .= "<script>$(function(){ date('drp');date_m('mdrp');});$('#clearform').on('click', function () {
					$('#".$this->formid."').find('input:text').val('');
					$('#".$this->formid."').find('option:selected').val('');
					$('input:checkbox').removeAttr('checked');
					newtable_cancel('".$this->formid."','".$this->divid."','".$this->hal."','".$this->sortby."','".$this->orderby."');
				});</script>";
			}else{
				$out .=	'<div class="form-group">';
		...
		if(count($this->proses) > 0 && $this->show_menu){
			if($this->tipe_proses=="button"){
				...
				foreach ($this->proses as $a => $b){
					$stat=$b[3];
					if($b[6] == "menu" || $b[6] == ""){
						$out.="<a href=\"javascript:void(0)\" onclick=\"button_menu('".$this->formid."',this.id,'".end($this->breadcrumbs)['title']."')\" id=\"tb_menu".$this->formid.$m."\"
		...
		if (count($this->rows) > 0){
			if (count($this->heading) > 0)
			{
				...
				if($this->tipe_proses2!=""){
					$out .= "<th><span class=\"order\">AKSI</span></th>";
				}
		...
		if (count($this->rows) > 0){
			...
			foreach($this->rows as $row){
				...
				$status = explode("|",$stat);$validasi=explode("|",$valid);
				if($status[1]!=$validasi[1]){$bg='style="background:#f0f0f0"';}						
				$out .= $this->newline;	
				if($cls=="odd"){
					$cels = '<td '.$this->td_click.' '.$bg.' class="alt">';	
				}else{
					$cels = '<td '.$this->td_click.' '.$bg.' class="odd">';
				}
				$out .= $cels.$x.'</td>';
				...
				foreach($row as $rowz => $cell){
					if ( !in_array($rowz, $this->hiderows)){
						$nm = (in_array($rowz, $this->numberformat))? ' style="text-align:right"' : '';	
						$dat = $this->dateformat;
						if (!ctype_digit ($cell)) { $timestamp = strtotime ($cell); }
						else { $timestamp = $cell; }

						if($cls=="odd"){
							$cels = '<td '.$this->td_click.' '.$bg.$nm.' class="alt">';	
						}else{
							$cels = '<td '.$this->td_click.' '.$bg.$nm.' class="odd">';
						}
						$out .= $cels;
						if ($cell === ""){
							...
							if ( in_array($rowz, $this->numberformat)){
								$out .= number_format($cell);
							} else {
								//$key = array_search($rowz, array_column($dat, "kolom"));
								$key = $this->searchForId($rowz, $dat);
								if ($key !== null){
									$param = (isset($dat[$key][1]))?$dat[$key][1]:"d-m-Y";
									$date = ($timestamp!="")?date($param,$timestamp):$cell;
									$out .= html_entity_decode($date);
								} else {
