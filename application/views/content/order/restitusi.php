    ...
    <div class="box-tab m-b-0" id="rootwizard">
      <ul class="wizard-tabs">
        ...
        <li style="width:100%;"> <a data-toggle="" style="text-align:right">
          <?php if($act=='proses'){?><button type="button" class="btn btn-primary btn-icon" id="buti" onclick="<?php echo 'process_popup(\'form_data\',\'divtblrestitusi\',\''.$gd.'\');';?> return false;"><?php echo $label;?><i class="icon-check"></i></button><?php } ?>
          </a> </li>
      </ul>
      <div class="tab-content">
        <div class="tab-pane p-x-lg active" id="tab1">
          <form name="form_data" id="form_data" class="form-horizontal" role="form" action="<?php echo site_url('order/execute/'.$act.'/restitusi/'.$id); ?>" method="post" autocomplete="off" onsubmit="<?php echo ($act=='proses')?'process_popup(\'form_data\',\'divtblrestitusi\',\''.$gd.'\');':'save_popup(\'form_data\',\'divtblrestitusi\');';?> return false;">
			<?php if($act=='proses'){?>
            ...
            <div class="form-group">
              <label class="col-sm-4 control-label-left">EX NOTA <?php echo ($act!='proses')?"":'';?></label>
              <div class="col-sm-8">
                <input type="text" name="EX_NOTA" value="<?php echo $arrhdr['EX_NOTA']; ?>" wajib="yes" id="EX_NOTA" class="form-control" placeholder="EX NOTA" maxlength="50" readonly>
              </div>
            </div>
			<div class="form-group">
			  <label class="col-sm-4 control-label-left">JENIS TRANSAKSI</label>
			  <div class="col-sm-8">
				<input type="hidden" name="JENIS_TRANSAKSI" value="<?php echo $arrhdr['JENIS_TRANSAKSI']; ?>" id="JENIS_TRANSAKSI" class="form-control" readonly> 
				<input type="text" name="JENIS_T" value="<?php echo $arrhdr['JENIS_T']; ?>" id="JENIS_T" placeholder="JENIS TRANSAKSI" class="form-control" readonly> 
			  </div>
			</div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="NAMA_KAPAL" id="NAMA_KAPAL" wajib="yes" placeholder="NAMA KAPAL" value="<?php echo $arrhdr['NM_ANGKUT']; ?>" maxlength="50" readonly<?php //echo ($act=='proses')?'readonly':'';?>>
              </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              <label class="col-sm-4 control-label-left">GUDANG</label>
              <div class="col-sm-2">
                <input type="text" class="form-control" name="KD_GUDANG_TUJUAN" id="KD_GUDANG_TUJUAN" wajib="yes" placeholder="KD_GUDANG_TUJUAN" value="<?php echo $arrhdr['KD_GUDANG_TUJUAN']; ?>" maxlength="4" readonly>
              </div>
              <div class="col-sm-6">
                <input type="<?php echo ($act=='proses')?'text':'text';?>" class="form-control" name="GUDANG" id="GUDANG_TUJUAN" wajib="yes" placeholder="GUDANG" value="<?php echo $arrhdr['NM_GUDANG']; ?>" maxlength="50">
              </div>
            </div>
			...
			<div class="text-center">
				...
				<button type="button" class="btn btn-primary btn-icon" id="buti" onclick="<?php echo 'save_popup(\'form_data\',\'divtblrestitusi\');';?> return false;"><?php echo 'SAVE';?><i class="icon-check"></i></button>
			</div>
			<?php } ?>
          </form>
        </div>
...
$(function(){
  date('drp');
  past_date('pdrp');
	  autocomplete('NO_BL_AWB','/autocomplete/ppbarang/no_bl/order',function(event, ui){
		$('#NO_MASTER_BL_AWB').val(ui.item.NO_MASTER_BL_AWB);
		$('#TGL_MASTER_BL_AWB').val(ui.item.TGL_MASTER_BL_AWB);
		$('#NO_BL_AWB').val(ui.item.KD_GUDANG_TUJUAN);
		...
		$('#JENIS_TRANSAKSI').val(ui.item.JENIS_TRANSAKSI);
		$('#JENIS_T').val(ui.item.JENIS_T);
		$('#NO_POLISI_TRUCK').val(ui.item.NO_POLISI_TRUCK);
		$('#EX_NOTA').val(ui.item.EX_NOTA);
		$('#TGL_KELUAR_LAMA').val(ui.item.TGL_KELUAR_LAMA);
		...
		$('#TGL_DO').val(ui.item.TGL_DO);
		$('#TGL_EXPIRED_DO').val(ui.item.TGL_EXPIRED_DO);
