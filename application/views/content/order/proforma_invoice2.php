...
foreach ($resulte as $row) {
	$tableheadercontainer .= ($p%15==0)?'<td style="vertical-align:top">':'';
	$tableheadercontainer .= $p+1 .'. '.$row['NO_CONT'].' ['.$row['UKURAN_CONT'].']<br>';
	$tableheadercontainer .= ($p%15==14)?'</td>':'';
	$p+=1;
}
$tableheadercontainer .= ($p%15!=0)?'</td>':'';
//$tableheadercontainer .= '</td>';
		...
		<table>
			<tr>
				<td>
					...
					<table>
						<tr>
							<td><h1 style="font-weight:bold">PROFORMA INVOICE PELAYANAN JASA : LCL PETIKEMAS</td>
						</tr>
					</table>
					...
					<table>
						...
						<tr>
						  <td>Nama Pemilik</td>
						  <td>:</td>
						  <td colspan=5><h2>'.$data['CONSIGNEE'].'</h2></td>
						</tr>
						<tr>
						  <td>NPWP Pemilik</td>
						  <td>:</td>
