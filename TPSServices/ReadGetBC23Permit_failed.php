...
//if (!$CheckFile) {
   // $createFile = $main->createFile($filename);
    ...
    //BEGIN
    //$SQL = "SELECT a.ID, a.RESPONSE FROM app_log_services a WHERE a.METHOD in ('GetBC23Permit','GetSppb_Bc23','GetBC23Permit_FASP')  AND a.FL_USED = '0' AND a.RESPONSE is not null order by a.WK_REKAM ASC limit 5";
	$idper='588421';
    $SQL = "SELECT a.ID, a.RESPONSE FROM app_log_services_success a WHERE a.METHOD in ('GetBC23Permit','GetSppb_Bc23','GetBC23Permit_FASP') and a.ID='".$idper."'";
    //$SQL = "SELECT * FROM app_log_services_success a where a.METHOD='GetBC23Permit_FASP' and a.WK_REKAM > str_to_date('19-11-2017','%d-%m-%Y')";
    ...
    if ($Query->size() > 0) { 
        while ($Query->next()) {
            ...
            if (count($xml) > 0) {
                ...
                if($countSPPB > 0){
                    $SQL = "INSERT INTO app_log_services_test_cfs SELECT * FROM app_log_services_success WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);
                    $SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN='UPDATE', WK_USED = NOW() WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);
                }else{
                    $SQL = "INSERT INTO app_log_services_test_cfs SELECT * FROM app_log_services_success WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);
                    $SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN = 'ERROR PARSING', WK_USED = NOW() WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);
                }
            }else{
				$SQL = "INSERT INTO app_log_services_test_cfs SELECT * FROM app_log_services_success WHERE ID = '" . $ID_LOG . "'";
				$Execute = $conn->execute($SQL);
				$SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN = 'ERROR NOT FOUND', WK_USED = NOW() WHERE ID = '" . $ID_LOG . "'";
				$Execute = $conn->execute($SQL);
            }
