...
// if (!$CheckFile) {
    ...
    $SQL = "SELECT A.ID, A.JENIS_BILLING, A.NO_ORDER,A.NO_INVOICE, C.BANK, B.CUSTOMER_NUMBER AS ID_ORGANISASI, IFNULL(B.NAMA_FORWARDER,B.CONSIGNEE) AS NAMA, IFNULL(B.ALAMAT_FORWARDER,B.ALAMAT_CONSIGNEE) AS ALAMAT, IFNULL(B.NPWP_FORWARDER,B.NPWP_CONSIGNEE) AS NPWP, A.SUBTOTAL, A.PPN, A.TOTAL, DATE_FORMAT(C.TGL_TERIMA, '%d/%m/%Y %H:%i:%s') AS TGL_TERIMA, C.APPR_CODE, C.REFF_NO, B.NM_ANGKUT, 
        ...
        WHERE A.MESSAGE_RECEIPT like '%FAIL%' AND A.MESSAGE_AR like '%SUCCESS%'";
    ...
    if ($Query->size() > 0) {
        ...
        while ($Query->next()) {
            ...
            $SQLpersentage = "SELECT SHARE_PERCENTAGE FROM mst_share_percentage WHERE KD_GUDANG = '". $KD_GUDANG_TUJUAN ."' AND DATE_FORMAT(NOW(),'%d-%m-%Y')>=DATE_FORMAT(START_DATE,'%d-%m-%Y')";
                    ...
                    <soapenv:Header/>
                        <soapenv:Body>
                           <ser:saveIntegrasi soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
							  ...
							  <type xsi:type="xsd:string">AR</type>
                           </ser:saveIntegrasi>
                        </soapenv:Body>
                     </soapenv:Envelope>';
    ...
    if ($Send['response'] != '') {
    	...
    	if ($countcomponent > 1) {
    		...
    		if (strpos($istrue, 'false') !== FALSE) {
            	...
            	//success
                $SQL = "UPDATE t_billing_cfshdr SET IS_SENDSIMKEU = '500' WHERE NO_INVOICE = '" . $transaction_numbertrx . "'";
        		$Execute = $conn->execute($SQL);
            }
    	}
    } else {
