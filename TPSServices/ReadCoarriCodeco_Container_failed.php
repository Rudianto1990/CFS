...
//BEGIN
  $SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_failed a WHERE a.METHOD in ('CoarriCodeco_Container') and a.RESPONSE like '%berhasil%' order by a.WK_REKAM ASC limit 100";
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_test_cfs a WHERE a.METHOD in ('CoarriCodeco_Container') and a.RESPONSE like '%berhasil%' order by a.WK_REKAM ASC limit 100";
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_failed a WHERE a.METHOD in ('CoarriCodeco_Container') and a.USERNAME='MTI' order by a.WK_REKAM ASC";
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_test_cfs a WHERE a.METHOD in ('CoarriCodeco_Container') order by a.WK_REKAM ASC";
...
if ($Query->size() > 0) {
        ...
        if (count($xml) > 0) {
            ...
            if ($countSPPB > 1) {
                for ($c = 0; $c < $countSPPB; $c++) {
                    ...
                    $retr = Insertcocostscont($KodeDokBC, $cocostscont, $ID_LOG);
					$retr = explode("|",$retr);
					$jml += $retr[1];
					$idd[] = $retr;
                }
    ...
    echo '<br>Jumlah seluruh CONT = '.$jml.'<hr>';
    }
	print_r($idd);
} else {
    echo 'data tidak ada.';
...
function InsertKontainer($ID, $CONT, $KD_DOK, $CFS="") {
    ...
    return ($Execute != '') ? 'true|'.$q.'|'.$NO_CONT : 'false|' . $SQL;
}
