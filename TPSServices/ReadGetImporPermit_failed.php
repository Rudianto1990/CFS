...
//if (!$CheckFile) {
   // $createFile = $main->createFile($filename);
    ...
    //BEGIN
    //$SQL = "SELECT a.ID, a.RESPONSE FROM app_log_services_failed a WHERE a.METHOD in ('GetImporPermit','GetImpor_Sppb','GetImporPermit_FASP') order by a.WK_REKAM ASC limit 5";
	$idper='1326564';
    $SQL = "SELECT a.ID, a.RESPONSE FROM app_log_services_success a WHERE a.METHOD in ('GetImporPermit','GetImpor_Sppb','GetImporPermit_FASP') 
	and a.ID='".$idper."'";
	/* $SQL = "select a.ID, a.RESPONSE from app_log_services_success a 
where a.USERNAME='MTI' and a.METHOD in ('GetImporPermit','GetImpor_Sppb','GetImporPermit_FASP')
and date_format(a.WK_REKAM, '%Y-%m-%d') >= '2017-12-20'"; */
            ...
            if (count($xml) > 0) {
                ...
                if ($countSPPB > 1) {
                    for ($c = 0; $c < $countSPPB; $c++) {
                        $SPPB = $xml['SPPB'][$c]['_c'];
                        InsertSPPB($KodeDokBC, $SPPB, $ID_LOG);
                    }
                } elseif ($countSPPB == 1) {
                    $SPPB = $xml['SPPB']['_c'];
                    InsertSPPB($KodeDokBC, $SPPB, $ID_LOG);
                ...
                if($countSPPB > 0){
                    $SQL = "INSERT INTO app_log_services_success SELECT * FROM app_log_services_failed WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);

                    $SQL = "UPDATE app_log_services_success SET KETERANGAN = 'Berhasil Parsing Data', WK_USED = NOW() WHERE ID = '" . $ID_LOG . "'";
                    $Execute = $conn->execute($SQL);
...
function InsertSPPB($KodeDokBC, $SPPB, $ID_LOG) {		
    global $CONF, $conn;
    ...
    $NAMA_IMP = trim($header['NAMA_IMP']['_v']) == "" ? "NULL" : "'" . mysql_real_escape_string(strtoupper(trim($header['NAMA_IMP']['_v']))) . "'";
    $ALAMAT_IMP = trim($header['ALAMAT_IMP']['_v']) == "" ? "NULL" : "'" . mysql_real_escape_string(strtoupper(trim($header['ALAMAT_IMP']['_v']))) . "'";
    ...
    $ALAMAT_PPJK = trim($header['ALAMAT_PPJK']['_v']) == "" ? "NULL" : "'" . mysql_real_escape_string(strtoupper(trim($header['ALAMAT_PPJK']['_v']))) . "'";
    ...
    if ($Query->size() == 0) {
        $SQL = "INSERT INTO t_permit_hdr (CAR, KD_KANTOR, KD_DOK_INOUT, NO_DOK_INOUT, TGL_DOK_INOUT, 
                                          ...
                                          FL_KARANTINA, KD_STATUS, TGL_STATUS,ID_LOG)
