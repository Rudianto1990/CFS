...
$main = new main($CONF, $conn);
  ...
  //BEGIN
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_failed a WHERE a.METHOD in ('CoarriCodeco_Kemasan') and a.RESPONSE like '%berhasil%' order by a.WK_REKAM ASC limit 100";
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_test_cfs a WHERE a.METHOD in ('CoarriCodeco_Kemasan') and a.RESPONSE like '%berhasil%' order by a.WK_REKAM ASC limit 100";
  $SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_failed a WHERE a.METHOD in ('CoarriCodeco_Kemasan') and a.USERNAME='MTI' and a.ID='1328672' order by a.WK_REKAM ASC";
  //$SQL = "SELECT a.ID, a.REQUEST, a.RESPONSE FROM app_log_services_test_cfs a WHERE a.METHOD in ('CoarriCodeco_Kemasan') order by a.WK_REKAM ASC";
  ...
  if ($Query->size() > 0) {
    while ($Query->next()) {
	  ...
	  if (count($xml) > 0) {
		...
		if ($countSPPB > 1) {
		  for ($c = 0; $c < $countSPPB; $c++) {
			...
			$jml = Insertcocostskms($KodeDokBC, $cocostskms, $ID_LOG);
			$idd[] = $jml;
		  }
		} elseif ($countSPPB == 1) {
		  ...
		  $SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN = 'Data XML salah' WHERE ID = '" . $ID_LOG . "'";
		  $Execute = $conn->execute($SQL);
		...
		echo 'XML tidak ada<br>';
		  ...
		  $SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN = 'XML tidak ada' WHERE ID = '" . $ID_LOG . "'";
		  $Execute = $conn->execute($SQL);
      ...
      echo '<br>Jumlah seluruh KMS = '.$jml.'<hr>';
    }
	print_r($idd);
  } else {
    echo 'data tidak ada.';
...
function Insertcocostskms($KodeDokBC, $cocostskms, $ID_LOG) {
    ...
    if ($Query->size() == 0) {
		...
		if($Execute!=''){
			...
			$sqlerror='Query Error: Cannot Insert t_cocostshdr; '.mysql_errno() . ": " . mysql_error().'; '.$SQL . '<hr>';
			echo $sqlerror;
			$SQL = "INSERT INTO app_log_services_test_cfs SELECT * FROM app_log_services_failed WHERE ID = '" . $ID_LOG . "'";
			$Execute = $conn->execute($SQL);
	...
	if ($ID != '') {
		...
		if($sqlerror==''){
			echo '<hr>sukses';
			$pindah = '';
			$SQL = "select * from app_log_services_success A where A.REQUEST like '%".$header['REF_NUMBER']['_v']."%'";
			$Query = $conn->query($SQL);
			if ($Query->size() > 0) {
				$pindah = ' delete';
			}
			...
			$SQL = "UPDATE app_log_services_test_cfs SET KETERANGAN = 'sukses ".$ss.$pindah."' WHERE ID = '" . $ID_LOG . "'";
			$Execute = $conn->execute($SQL);
...
function InsertKemasan($ID, $KMS, $KD_DOK) {
	...
	$BRUTO = floatval(str_replace(',', '.', str_replace(',', '', $BRUTO)));
	$NO_BC11 = trim($KMS['NO_BC11']['_v']) == "" ? "NULL" : "'" . strtoupper(trim($KMS['NO_BC11']['_v'])) . "'";
	$TGL_BC11 = trim($KMS['TGL_BC11']['_v']) == "" ? "NULL" : "STR_TO_DATE('" . strtoupper(trim($KMS['TGL_BC11']['_v'])) . "','%Y%m%d')";
    ...
    return ($Execute != '') ? 'true|'.$q.'|'.$NO_BL_AWB : 'false|' . $SQL;
}
