...
// if (!$CheckFile) {
                     ...
                     <soapenv:Header/>
                         <soapenv:Body>
                            <ser:saveIntegrasi soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
				 			  ...
				 			  <type xsi:type="xsd:string">AP</type>
                            </ser:saveIntegrasi>
                         </soapenv:Body>
                      </soapenv:Envelope>';
    ...
    if ($Send['response'] != '') {
    	...
    	if ($countcomponent > 1) {
    		...
    		for ($i=0; $i < $countcomponent; $i++) { 
	    		...
	    		$message .= $messagepay;
                $SQL = "UPDATE t_billing_cfshdr SET IS_SENDSIMKEU = '200',STATUS_AP = NULL WHERE NO_NOTA = '" . $transaction_numbertrx . "'";
                $Execute = $conn->execute($SQL);

	    		if (strpos($istrue, 'false') !== FALSE) {
	    			//false
	                $SQL = "UPDATE t_billing_cfshdr SET IS_SENDSIMKEU = '400',STATUS_AP = NULL WHERE NO_NOTA = '" . $transaction_numbertrx . "'";
	        		$Execute = $conn->execute($SQL);
	            } else {
	            	//success
	                $SQL = "UPDATE t_billing_cfshdr SET IS_SENDSIMKEU = '300',STATUS_AP = NULL WHERE NO_NOTA = '" . $transaction_numbertrx . "'";
	        		$Execute = $conn->execute($SQL);
	            }
    		}
    		...
    		if (strpos($istrue, 'false') !== FALSE) {
            	...
            	//success
                $SQL = "UPDATE t_billing_cfshdr SET IS_SENDSIMKEU = '500',STATUS_AP = NULL WHERE NO_NOTA = '" . $transaction_numbertrx . "'";
        		$Execute = $conn->execute($SQL);
            }
    	}
    } else {
	...
	$Password = 'SIMKEU';
	$url = 'ipccfscenter.com/TPSServices/';
	$method = 'saveTransactionAP';
	...
	$SQL = "INSERT INTO app_log_services (USERNAME, PASSWORD, URL, METHOD, REQUEST, RESPONSE, IP_ADDRESS, WK_REKAM)
			VALUES ('" . $userName . "','" . $Password . "','" . $url . "','" . $method . "','" . mysql_real_escape_string($message) . "','" . mysql_real_escape_string($response) . "','" . $ipAddress . "', NOW())";
	$Execute = $conn->execute($SQL);
	if(!$Execute){
