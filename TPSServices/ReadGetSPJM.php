...
//if (!$CheckFile) {
   // $createFile = $main->createFile($filename);
	...
	('GetSPJM','GetSPJM_onDemand','ReceiveSPJM','ReceiveSPJM_onDemand')  
            ...
            if (count($xml) > 0) {
                ...
                if ($countSPJM > 1) {
                    for ($c = 0; $c < $countSPJM; $c++) {
                        $SPJM = $xml['SPJM'][$c]['_c'];
                        InsertSPJM($KodeDokBC, $SPJM, $ID_LOG);
                    }
                } elseif ($countSPJM == 1) {
                    $SPJM = $xml['SPJM']['_c'];
                    InsertSPJM($KodeDokBC, $SPJM, $ID_LOG);
...
function InsertSPJM($KodeDokBC, $SPJM, $ID_LOG) {
    global $CONF, $conn;
